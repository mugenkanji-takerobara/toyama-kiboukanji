// ====== 安全版：DOM と要素存在チェックを徹底したゲーム本体（先頭に置く） ======

// 音や変数はここで宣言だけして、実体化は DOMContentLoaded 内で行う
let waveBGM = null;
let storyBGM = null;
let shamisenIntro = null;

// ゲーム用変数（初期化は startGame 内で）
...
function safeAddListener(id, ev, fn){ const e = $(id); if(e) e.addEventListener(ev, fn); }

// 画面切替（既存 HTML は .screen と .screen.active を使っている想定）
...
// ゲームロジック（描画等）は ctx があるときだけ動くようにする
let canvas = null;
let ctx = null;

// createKanji / gameLoop / startGame / endGame を定義（ctx が null の場合は安全に抜ける）
...
function startGame(){
  ...
  // canvas と ctx を確実に取得（DOM がある前提でここは呼ばれる）
  canvas = $("game-canvas");
...
function updateStoryBGM(pageIndex){
  if(pageIndex <= 5) fadeIn(storyBGM); else fadeOut(storyBGM);
}

// DOM が完全に準備できてから要素を作る / イベントを登録する
document.addEventListener("DOMContentLoaded", () => {

  // --- 音の実体化（存在しないファイルでも try/catch で安全化） ---
  try { waveBGM = new Audio("audio/穏やかな波.mp3"); waveBGM.loop = true; waveBGM.volume = 0.01; } catch(e){ waveBGM = null; }
  try { storyBGM = new Audio("audio/shamisen_intro.mp3"); storyBGM.loop = true; storyBGM.volume = 0.02; } catch(e){ storyBGM = null; }
  try { shamisenIntro = new Audio("audio/shamisen_intro.mp3"); shamisenIntro.volume = 0.02; } catch(e){ shamisenIntro = null; }
  ...
  const bgmEl = $("bgm");
  function playBGM(){
    if(bgmEl && bgmEl.paused){ bgmEl.volume = 0.01; bgmEl.play().catch(()=>{}); }
    else if(waveBGM && waveBGM.paused){ waveBGM.currentTime = 0; waveBGM.play().catch(()=>{}); }
  }

  // --- 動的読み込み関数（必要なら game.js から呼ぶ） ---
  if(typeof loadMainGameScript !== "function"){
    window.loadMainGameScript = function(cb){
      if(window.mainGameLoaded) return cb && cb();
      const s = document.createElement("script");
      s.src = 'toyama-kiboukanji-j.js'; // 実際のファイル名に合わせる
      s.defer = true;
      s.onload = ()=>{ window.mainGameLoaded = true; cb && cb(); };
      s.onerror = ()=>{ console.error('failed to load', s.src); cb && cb(); };
      document.head.appendChild(s);
    };
  }

  // --- Start ボタン（直接 startGame を呼ばない安全版） ---
  const startBtn = $("start-button");
  if(startBtn){
    startBtn.addEventListener("click", ()=>{
      playBGM();
      // 既に本体が同ファイル内にあるなら直接 startGame を呼ぶ
      if(typeof startGame === "function"){
        showScreen("game-screen");
        startGame();
      } else {
        // 動的読み込みしてから startGame を呼ぶ（loadMainGameScript は上で定義済み）
        loadMainGameScript(()=>{
          showScreen("game-screen");
  ...
  // --- Manual（説明書）関連（要素が無ければ何もしない） ---
  safeAddListener("manual-button","click", ()=>{
    playBGM();
    const overlay = $("manualOverlay");
    if(overlay) overlay.classList.remove("hidden");
    if(typeof loadManualPage === "function") loadManualPage(0);
  });
  safeAddListener("close-manual","click", ()=>{
    const overlay = $("manualOverlay");
    if(overlay) overlay.classList.add("hidden");
  });
  safeAddListener("back-to-title","click", ()=>{
    showScreen("title-screen");
    if(waveBGM){ waveBGM.pause(); waveBGM.currentTime = 0; }
  });

  // --- story / toyama ボタン（存在すれば） ---
  safeAddListener("toyama-button","click", ()=>{
    showScreen("toyamaScreen");
    if(typeof loadStoryPage === "function") loadStoryPage(0);
  });

  // --- story 言語切替（存在すれば） ---
  const langJa = $("lang-ja"), langEn = $("lang-en");
  if(langJa) langJa.addEventListener("click", ()=>{ document.querySelectorAll(".story-ja").forEach(e=>e.style.display="block"); document.querySelectorAll(".story-en").forEach(e=>e.style.display="none"); });
  if(langEn) langEn.addEventListener("click", ()=>{ document.querySelectorAll(".story-ja").forEach(e=>e.style.display="none"); document.querySelectorAll(".story-en").forEach(e=>e.style.display="block"); });

  // --- canvas タッチイベント（安全に登録） ---
  canvas = $("game-canvas");
  ctx = canvas ? canvas.getContext("2d") : null;
  if(canvas){
    ...
    canvas.addEventListener("touchend", e=>{
      ...
      // ゲームロジックの関数がある場合のみ呼ぶ
      if(typeof handleBonusTap === "function" && typeof bonusMode !== "undefined" && bonusMode && dist < 20){ handleBonusTap(t); return; }
      // その他の操作はゲーム本体に任せる
    });
  }

  // --- 開始時の軽いループを回す（描画は startGame 内で始める） ---
  requestAnimationFrame(()=>{ /* noop to keep rAF warm */ });
